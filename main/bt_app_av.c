...
/* MAIN_UI_BRIDGE_ACTIVE - direct AVRCP to LVGL bridge */
    if (event == ESP_AVRC_CT_CHANGE_NOTIFY_EVT) {
        uint8_t ui_evt = rc->change_ntf.event_id;

        if (ui_evt == ESP_AVRC_RN_PLAY_POS_CHANGED) {
            uint32_t pos = rc->change_ntf.event_parameter.play_pos;
            ESP_LOGI(BT_AV_TAG, "MAIN_UI_BRIDGE position=%lu-ms", (unsigned long)pos);
            garage_ui_set_track_position(pos);
} else if (ui_evt == ESP_AVRC_RN_PLAY_STATUS_CHANGE) {
            uint8_t st = rc->change_ntf.event_parameter.playback;
            ESP_LOGI(BT_AV_TAG, "MAIN_UI_BRIDGE playback=0x%x", st);
            garage_ui_set_playback_status_raw(st);
} else if (ui_evt == ESP_AVRC_RN_TRACK_CHANGE) {
            ESP_LOGI(BT_AV_TAG, "MAIN_UI_BRIDGE track changed");
            garage_ui_track_changed();
}
    }

    if ((int)event == 3 || event == ESP_AVRC_CT_PLAY_STATUS_RSP_EVT) {
        ESP_LOGI(BT_AV_TAG, "MAIN_UI_BRIDGE play status len=%lu pos=%lu status=0x%x",
                 (unsigned long)rc->play_status_rsp.song_length,
                 (unsigned long)rc->play_status_rsp.song_position,
                 rc->play_status_rsp.play_status);

        if (rc->play_status_rsp.song_length != 0xFFFFFFFFUL) {
            garage_ui_set_track_duration(rc->play_status_rsp.song_length);
        }

        if (rc->play_status_rsp.song_position != 0xFFFFFFFFUL) {
            garage_ui_set_track_position(rc->play_status_rsp.song_position);
        }

        garage_ui_set_playback_status_raw(rc->play_status_rsp.play_status);
    }

    if (event == ESP_AVRC_CT_METADATA_RSP_EVT &&
        rc->meta_rsp.attr_id == ESP_AVRC_MD_ATTR_PLAYING_TIME &&
        rc->meta_rsp.attr_text &&
        rc->meta_rsp.attr_text[0]) {
        uint32_t dur = (uint32_t)strtoul((const char *)rc->meta_rsp.attr_text, NULL, 10);
        ESP_LOGI(BT_AV_TAG, "MAIN_UI_BRIDGE metadata duration=%lu-ms", (unsigned long)dur);
        garage_ui_set_track_duration(dur);
    }
