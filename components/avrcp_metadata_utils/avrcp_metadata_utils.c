...
void bt_avrc_md_ct_evt_hdl(uint16_t event, void *param)
{
    ...
    case ESP_AVRC_CT_METADATA_RSP_EVT: {
        ...
        if (rc->meta_rsp.attr_id == ESP_AVRC_MD_ATTR_PLAYING_TIME &&
            rc->meta_rsp.attr_text && rc->meta_rsp.attr_text[0]) {
            uint32_t duration_ms = (uint32_t)strtoul((const char *)rc->meta_rsp.attr_text, NULL, 10);
            ESP_LOGI(BT_RC_CT_TAG, "Track duration from metadata: %"PRIu32"-ms", duration_ms);
            garage_ui_set_track_duration(duration_ms);
        }
