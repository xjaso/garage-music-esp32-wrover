...
static esp_err_t avrc_cover_art_srv_decode_image(void)
{
    ...
    s_avrc_cover_art_srv_cb.pixels = heap_caps_calloc(1, outimg.output_len, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!s_avrc_cover_art_srv_cb.pixels) {
        s_avrc_cover_art_srv_cb.pixels = heap_caps_calloc(1, outimg.output_len, MALLOC_CAP_8BIT);
    }
...
static void avrc_cover_art_srv_display_image(void)
{
    if (s_avrc_cover_art_srv_cb.pixels && s_avrc_cover_art_srv_cb.decoded_w && s_avrc_cover_art_srv_cb.decoded_h) {
        ...
        /* garage_lvgl.c copies/scales the image into its persistent 84x84 LVGL buffer.
         * After that, release temporary JPEG decode/download buffers so the next cover has RAM. */
        avrc_cover_art_srv_free_pixels();
...
void avrc_cover_art_srv_close(void)
{
    ...
    s_avrc_cover_art_srv_cb.image_save_failed = false;
    avrc_cover_art_srv_free_image_data();
    avrc_cover_art_srv_free_pixels();
    garage_ui_set_status("WAIT");
...
void avrc_cover_art_srv_set_image_final(bool final)
{
    ...
    if (s_avrc_cover_art_srv_cb.image_final) {
        ...
        if (s_avrc_cover_art_srv_cb.image_save_failed ||
            !s_avrc_cover_art_srv_cb.image_data ||
            s_avrc_cover_art_srv_cb.image_size == 0) {
            ESP_LOGW(RC_CA_SRV_TAG, "Skip cover decode: incomplete JPEG buffer, size=%lu, save_failed=%d",
                     s_avrc_cover_art_srv_cb.image_size,
                     s_avrc_cover_art_srv_cb.image_save_failed);
            avrc_cover_art_srv_free_image_data();
            avrc_cover_art_srv_free_pixels();
            s_avrc_cover_art_srv_cb.image_save_failed = false;
            s_avrc_cover_art_srv_cb.getting = false;
...
void avrc_cover_art_srv_save_image_data(uint8_t *p_data, uint16_t data_len)
{
    ...
    if (!p_buf) {
        ...
        avrc_cover_art_srv_free_image_data();
        s_avrc_cover_art_srv_cb.image_save_failed = true;
        s_avrc_cover_art_srv_cb.getting = false;
        return;
    }
