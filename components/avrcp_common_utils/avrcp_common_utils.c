...
void bt_avrc_common_ct_evt_def_hdl(uint16_t event, void *param)
{
    ...
    case ESP_AVRC_CT_METADATA_RSP_EVT: {
        ...
        ESP_LOGI(BT_RC_CT_TAG, "Play status response handled in main bridge");
        break;
...
void bt_avrc_common_ct_rn_play_pos_changed(void)
{
    ...
    if (esp_avrc_rn_evt_bit_mask_operation(ESP_AVRC_BIT_MASK_OP_TEST, &s_avrc_common_cb.peer_rn_cap,
        ...
        esp_avrc_ct_send_register_notification_cmd(bt_avrc_common_alloc_tl(), ESP_AVRC_RN_PLAY_POS_CHANGED, 1);
    }
}
...
bt_avrc_common_ct_rn_track_changed();
    ...
    case ESP_AVRC_RN_PLAY_STATUS_CHANGE:
        ...
        garage_ui_set_playback_status_raw(event_parameter->playback);
bt_avrc_common_ct_rn_play_status_changed();
        break;
    /* when track playing position changed, this event comes */
    case ESP_AVRC_RN_PLAY_POS_CHANGED:
        ESP_LOGI(BT_RC_CT_TAG, "Play position changed: %"PRIu32"-ms CALL_UI_PATCH_ACTIVE", event_parameter->play_pos);
        garage_ui_set_track_position(event_parameter->play_pos);
        ESP_LOGI(BT_RC_CT_TAG, "AFTER garage_ui_set_track_position");
